function structVol = ElementVolLS(elementNode,nodeCoor,valLS,varargin)
...
structVol = zeros(nEle,1);
if ~isempty(varargin) && strcmp(varargin{1},'group')
    elementGroup = varargin{2};
end
for iEle = 1:nEle
    ...
    if iscell(valLS)
        iG = elementGroup(iEle,1);
        eleNodeLS =  valLS{iG}(eleNode,1);
    else
        eleNodeLS =  valLS(eleNode,1);
    end
    if min(eleNodeLS)>=0
        eleVol = ElementVol([],eleNodeCoor,2);
