function [eleK,eleVol] = ElementK_Density(penal,eleNodeCoor,eleNodeLS,eleMat,eleProfile)
...
if min(eleNodeLS)>=0
    ...
    eleVol = abs(a(:,1).*b(:,2)-a(:,2).*b(:,1))/2+abs(c(:,1).*d(:,2)-c(:,2).*d(:,1))/2;
...
eleK = ElementK(eleNodeCoor,eleMat,eleProfile);
end